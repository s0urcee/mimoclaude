...
.USAGE
    mimoclaude                       # MiMo V2.5 Pro (default)
    mimoclaude --backend or          # OpenRouter
    mimoclaude --backend fw          # Fireworks AI (fastest)
...
if (-not $Backend -and -not $Status -and -not $Cost -and -not $Benchmark -and -not $Help) {
    $Backend = if ($env:MIMOCLAUDE_DEFAULT_BACKEND) { $env:MIMOCLAUDE_DEFAULT_BACKEND } else { "mimo" }
}
...
if ($Status) {
    ...
    Write-Host "    mimoclaude              # MiMo V2.5 Pro (default)"
    Write-Host "    mimoclaude -b or        # OpenRouter"
    Write-Host "    mimoclaude -b fw        # Fireworks AI (fastest)"
...
# --- Cost ---
if ($Cost) {
    Write-Host "`n  MiMo V2.5 Pricing (99% price cut May 2026)" -ForegroundColor Cyan
    Write-Host "  ==========================================" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Provider        Input/M    Output/M   Cache Hit/M" -ForegroundColor Yellow
    Write-Host "  ----------      --------   --------   -----------"
    Write-Host "  MiMo V2.5       `$0.14      `$0.28      (provider)" -ForegroundColor Green
    Write-Host "  MiMo V2.5 Pro   `$0.435     `$0.87      (provider)" -ForegroundColor Green
    Write-Host "  OpenRouter      `$0.44      `$0.87      (provider)"
    Write-Host "  Fireworks       `$1.74      `$3.48      (provider)"
    Write-Host "  Anthropic       `$3.00      `$15.00     `$0.30"
    Write-Host ""
    Write-Host "  Monthly estimate (heavy use): `$15-50 vs `$200 Anthropic" -ForegroundColor Green
    Write-Host ""
    exit 0
}
...
# --- Help ---
if ($Help) {
    Write-Host "mimoclaude — Claude Code with MiMo and alternative backends"
    Write-Host ""
    Write-Host "Usage: mimoclaude [-b backend] [--status] [--cost] [--benchmark]"
    Write-Host ""
    Write-Host "  -b, --backend   mimo (default), or, fw, anthropic"
...
$p = $Providers[$Backend]
if (-not $p) { Write-Host "ERROR: Unknown backend '$Backend'. Use: mimo, or, fw, anthropic" -ForegroundColor Red; exit 1 }
if (-not $p.key) { Write-Host "ERROR: $($p.keyName) not set" -ForegroundColor Red; exit 1 }
